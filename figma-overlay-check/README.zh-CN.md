...
- 叠图应提供“隐藏图片”“透明叠加”“展示图片”三种按钮和透明度控制，其中“展示图片”内部使用差值模式；刷新后仍可继续比较。
- 面板所有可见文字必须使用中文；标题栏可拖动，关闭按钮只把面板收起为浏览器边缘的一小块“叠图”浮标，不隐藏叠图图片，点击浮标恢复完整面板。
- 图片关闭模式命名为“隐藏图片”，原差值模式在界面中命名为“展示图片”；边缘浮标外露侧使用圆角。面板状态只保存在内存中，不写入任何浏览器存储，刷新后恢复为展开的“透明叠加”模式和 `0.5` 透明度。
