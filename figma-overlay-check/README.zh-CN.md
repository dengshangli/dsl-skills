...
7. 恢复并保留最终叠图供用户手动复查，记录叠图产物，并提示用户后续调用 `figma-overlay-cleanup`。
...
- 检查完成后会有意保留叠图，并在项目根目录写入 `.figma-overlay-state.json`。
- 手动复查完成后，调用 `$figma-overlay-cleanup` 或回复“删除叠图”，即可删除叠图及已记录的图片产物，同时保留 UI 修改。

## 完整规则

完整的七步工作流、通过标准、脚本用法、排错方法、叠图保留和清理交接规则，请查看 [SKILL.md](./SKILL.md)。
