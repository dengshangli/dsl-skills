...
- Label the image-off mode `隐藏图片` and the difference-backed image-on mode `展示图片`. Give the edge tab rounded exposed corners. Keep panel state in memory only; do not use browser storage, and reset to the expanded `透明叠加` mode at opacity `0.5` after refresh.
